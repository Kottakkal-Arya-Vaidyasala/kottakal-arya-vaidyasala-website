...
export async function sendContactEmail(
  ...
  if (!isConfigured) {
    console.log(
      "[Mock Email Sent] Dispatching to EmailJS mock layer:",
      formData
    );
    // Simulate API delay
  ...
  try {
    ...
    const templateParams = {
      ...
      selected_doctor:
        formData.doctor || "General Practitioner / Open Consultation",
    ...
    if (response.status === 200) {
      ...
      throw new Error(
        `EmailJS API response code: ${response.status} - ${response.text}`
      );
    }
  } catch (error: unknown) {
    ...
    return {
      success: false,
      message:
        "Failed to dispatch message. Please try again or contact our front office directly.",
